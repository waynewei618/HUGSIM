...
if __name__ == '__main__':
    ...
    with open(os.path.join(path_prior, 'cameras.txt'), 'w') as f:
        for idx, frame in enumerate(meta_data['frames'][:3]):
    ...
    #         if idx == 0:
    #             ref2w = c2w
    #         ref2c = np.linalg.inv(c2w) @ ref2w
    #         qvec = rotmat2qvec(ref2c[:3, :3])
    #         rigid_cam = {
    #             'camera_id': idx+1,
    #             'image_prefix': frame['rgb_path'].split('/')[-2]+'/',
    #             # 'rel_tvec': [ref2c[:3, 3].tolist()], # UC-NeRF defination is wierd
    #             # 'cam_from_rig_rotation': [ref2c[:3, 3].tolist()],
    #             # 'cam_from_rig_translation': [qvec.tolist()],
    #         }
    #         cam_rigid['cameras'].append(rigid_cam)
    # with open(path_rigid, 'w') as f:

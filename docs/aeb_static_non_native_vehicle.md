...
当前 `gui/app.py` 生成的是闭环仿真使用的 scenario yaml，后续由 `closed_loop.py` / `HUGSimEnv` 读取。它不是 `aeb_hil_vil_render/compose_compare_video.py` 的输入。

当前 AEB 前视离线渲染已经支持直接插入一辆静态非原生车辆。`GaussianSceneRenderer` 初始化时会把 `aeb_trajectory.json` 加载为 `self.ego_trajectory`，根据指定里程 `s` 从轨迹中插值得到车辆插入位置和朝向，再交给 `aeb_hil_vil_render/static_vehicle_insertion.py` 加载 `/data/realcar3d/<vehicle_id>/gs.pth`，并把它作为一个固定 `body_to_world` 动态对象合入每帧渲染。

...
本文档专门记录 `render_3dgs/core/lut_distortion.py`。该模块只负责读取 VTD distortion lookup table，并把已经渲染出的 pinhole 图像按 LUT 重映射为最终畸变图像。
...
```text
/workspace/HUGSIM/render_3dgs/data/vtd_front_120/front_120_parameters.json
```
...
distorter = LookupTableDistorter(
    "/workspace/HUGSIM/render_3dgs/data/vtd_front_120/front.dat"
)

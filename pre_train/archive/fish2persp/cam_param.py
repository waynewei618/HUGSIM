...
class CamParams(nn.Module):
    ...
    def from_file(pose_file: str, cam_pose_file: str, intrinsic_file: str, 
        ...
        print(poses[8+16*3])'''
        # phi, t, f
        phi_left = tr3d.matrix_to_quaternion(poses_left[:, :3, :3])
        # divide 50. to normalize the scene
        t_left = poses_left[:, :3, 3] / coord_scale_factor

        phi_right = tr3d.matrix_to_quaternion(poses_right[:, :3, :3])
        # divide 50. to normalize the scene
    ...
    @staticmethod
    def transform_pose_list(pose_file, inv, 
        ...
        render_poses = c2w_track_spiral(poses_center, up, rads, focus_distance, zrate=0.5, rots=2, N=30)
...
def get_rays_patch(
    ...
    if N_rays > 0:
        ...
        N_rays_x, N_rays_y = int(math.sqrt(N_rays)), int(math.sqrt(N_rays))
        start_x = int(np.random.randint(0,H - N_rays_x - 1))
        start_y = int(np.random.randint(0,W - N_rays_y - 1))

        _idxs = []

        for idx_x in range(start_x,start_x+N_rays_x):
            _idxs += [ W*idx_x + idx_y for idx_y in range(start_y,start_y+N_rays_y) ]

        _idxs = np.array(_idxs)

        '''print('_idxs ',_idxs,H*W)
        if np.any( _idxs > H*W ):
            print('Hello')'''

        select_inds = torch.from_numpy(_idxs.reshape([*prefix, N_rays])).to(device)
        #select_inds = torch.from_numpy(
        #    np.random.choice(H*W, size=[*prefix, N_rays], replace=False)).to(device)
        i = torch.gather(i, -1, select_inds)
        j = torch.gather(j, -1, select_inds)
